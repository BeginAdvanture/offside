...
function storeList(result){
	...
	let html = "";
		for(var i=0;i<result.length;i++) {
			...
			const matchName = store.matchName;
			...
			if(store.isOpen == "false") {
				isOpenHtml = `<div class="is_open">
								<a href="/store/detail/${matchId }">지금은 준비중입니다</a>
							</div>`;
			}
			...
			`<li >
				<div class="img_box">
					<a href="/store/detail/${matchId }"><img src="${storeImg }" alt="이미지"></a>
				</div>
				<div class="info_box">
					<h2><a href="/store/detail/${matchId }">${matchName }</a></h2>
					<a href="/store/detail/${matchId }">
						<span>
							<span>평점 ${score }</span>
